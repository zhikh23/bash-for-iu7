...
- [Учебная практика BASH для чайников](#учебная-практика-bash-для-чайников)
  ...
  - [Глава II. Строки — это всё](#глава-ii-строки--это-всё)
    ...
    - [2.5. Ввод переменных пользователем](#25-ввод-переменных-пользователем)
    - [2.6. Кавычки? Не, не слышали](#26-кавычки-не-не-слышали)
    - [2.7. Загадочный доллар](#27-загадочный-доллар)
    - [2.8. Позиционные переменные](#28-позиционные-переменные)
    - [2.9. Вопросы существования с точки зрения программиста](#29-вопросы-существования-с-точки-зрения-программиста)
  - [Глава III. Ветвления в bash-е или "А что, если..."](#глава-iii-ветвления-в-bash-е-или-а-что-если)
    ...
    - [3.6. Таинственные минусы](#36-таинственные-минусы)
    - [3.7. =~](#37-)
  ...
  - [Глава IV. Что такое регулярные выражения и с чем их есть?](#глава-iv-что-такое-регулярные-выражения-и-с-чем-их-есть)
    ...
    - [4.6. Якоря](#46-якоря)
    - [4.7. Числа!](#47-числа)
    - [4.8. С чем регулярные выражения путать нельзя или маски файлов](#48-с-чем-регулярные-выражения-путать-нельзя-или-маски-файлов)
...
Автор допускает, что в методическом пособии присутствуют ошибки, поэтому о них следует сообщать по контактам ниже:
...
> Интерпретатор в большинстве случаев не работает с переменными напрямую. Он проходит по строке и заменяет все указания переменных на их соответствующее значение, а _потом исполняет строку как код_.
...
Потому что вне зависимости от наличия кавычек, переменная всё равно будет, и будет она _строковой_.
...
Её значение равно _коду возврата последней выполненной команды_. Например:
```bash
some_program
echo $?
```
...
«Зачем это нужно?» Возвращаясь к теме обратной совместимости, такая запись использовалась в shell-е, а в bash перешла по наследству. 
...
Маски файлов похожи на регулярные выражения, но ими не являются. У масок значительно более простой синтаксис, т.к. они предназначены исключительно для поиска файлов.
Они имеют очень простой синтаксис:
...
Числа желательно проверять регулярным выражением, т.к. всё в bash-у является строкой и нет отдельного типа данных для чисел.
